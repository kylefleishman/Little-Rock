...
// handles the stabilization of floating large objects by using array of offset vectors instead of just one value
// TODO
// add logic to turn on interpolation only when needed, will make object interaction smoother and optimized
[RequireComponent(typeof(Rigidbody))]
public class CustomStabilizedRigidBodyPhysics : MonoBehaviour
{
	...
	// check to see if raycast fails to check whether point is inside water volume, if true set submergence to 1
	[SerializeField]
	...
	// to prevent arbitrary rotations while floating, simulate objects floating with their lightest side upwards with buoyancy offest ( array of offset vectors )
	...
	void FixedUpdate() {
		...
		// apply drag and buoyancy per offset, have to be divided by amount of offsets so max effect remains the same
		// actual effect experienced depends on submergence total
		...
		for (int i = 0; i < buoyancyOffsets.Length; i++) {
			...
			if (submergence[i] > 0f) {
				...
				customRigidBody.AddForceAtPosition(gravity * (buoyancyFactor * submergence[i]), transform.TransformPoint(buoyancyOffsets[i]), ForceMode.Acceleration);
	...
	// calculate all submergence buoyancy offsets seperately
	// only set safe floating toggle on large objects that can be pushed out of water
